...
try:
    policies = the ACS manifest.load_from_file("policies/my-policy.yaml")
...
def load_policies(policy_paths: list[str]) -> list[the ACS manifest]:
    policies = []
    for path in policy_paths:
        try:
            with open(path) as f:
                data = yaml.safe_load(f)
            policies.append(the ACS manifest.from_dict(data))
        except PolicyLoadError as e:
