...
| Haystack | Pipeline | `HaystackAdapter` | `AgentControlChecker` + `TrustGate` pipeline components. |
| Dify | Plugin | `DifyPlugin` | Install from Dify Marketplace. Zero-code governance. |
| Azure AI Foundry | Deployment Guide | MAF Middleware | `AgentControlMW`, `CapabilityGuardMW`, `AuditTrailMW`, `RogueDetectionMW`. |
...
```python
from agent_os.policies import AsyncAgentControl

evaluator = AsyncAgentControl(policy_dir="./policies")
...
- **Foundry agents** get native middleware integration (`AgentControlMW`, `CapabilityGuardMW`, `AuditTrailMW`) — governance is invisible and automatic.
