...
ALLOWED_V4_FILES: frozenset[str] = frozenset(
    {
        ...
        "agent-governance-python/agt-policies/tests/test_migrate_bridge.py",
        "agent-governance-python/agt-policies/tests/test_migrate_resolution.py",
        "docs/v4-removal.md",
        # The breaking-changes record has to name each removed symbol so
        # users can find its replacement. Naming a symbol in order to say it
        # is gone documents the removal; it is not a use of it.
        "BREAKING_CHANGES.md",
...
ALLOWED_V4_PREFIXES: tuple[str, ...] = (
    ...
    # Security audit records describe the code as it stood when each audit was
    # written. They are historical evidence tied to a merged PR, so they keep
    # naming the v4 API that existed at the time, the same way the removal plan
    # doc does. Rewriting them would falsify the record.
...
def decide_scanner(path: Path) -> str | None:
    ...
    if suffix in (".rs", ".ts", ".tsx"):
        return "tokens"
    # Notebooks ship as runnable examples. Scan the raw JSON for v4 names so a
    # notebook cannot quietly keep importing a symbol the runtime dropped.
    if suffix == ".ipynb":
        return "tokens"
